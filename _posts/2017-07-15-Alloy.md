...
The `sig` keyword defines a type, like a class in Java. However, in Alloy it is the name of an (infinite) _set_ of _atoms_. From the specification's perspective 

The entries represent the table of one object mapping to another object. The `lone` keyword should be read as _optional_, it means that for a given Object there is a mapping to a single Object or no mapping at all. 
