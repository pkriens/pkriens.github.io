...
I tried to ignore JPMS but my renewed love of formal specifications got the better of me. Trying to put JPMS in [Alloy][1] made me aqutely aware of how badly specified JPMS really is. There are  loose ends, errors, and confusing statements that seem not to faze anybody as far as I can see. Java is supposed to be the language of 8 million developers and 3 billion devices, should we not expect some quality control when a new foundation is laid?

In a series of articles I want to go through the [JPMS document][1], a document describing changes to the language and the VM which was the input to the failed vote. However, this is only a part, the rather old [State Of the Module System][3], also in Mark Reinhold's home directory, is the most up to date document I could find. This installment is purely about the JLS. later installments will dive into the runtime aspects of the  [State Of the Module System][3].

## About Alloy

Alloy is the first formal specification language I met that is quite readable, even for beginners, accompanied with a terrific tool to explore the model. Though it cannot provide a formal proof, it can efficiently completely traverse enormous state spaces to find (counter) examples of invariants or special cases in an interactive way. This is usually sufficient because most errors can be demonstrated with only a few instances. A good introduction of Alloy is the [Daniel Jackson's presentation to ICSE][4]. I will try to explain the different concepts used to make them readable for experienced Java developers.

Most important thing to remember is that Alloy (or formal specs in general) are like carving a statue. You start with the infinite state space of all possible combinations and you then put constraints to whittle down this space. That is, you can only remove because you start with the universe. As Java developers we always think opposite, we build things from smaller parts. For example, if a statement says `# foo = 4` then we are not _creating_ a set with 4 elements but we're throwing away all state where `foo` is not sized 4. Understanding that we're reasoning about an infinite state took me very long to internalise.

Learning to use formal specifications is quite tricky, I might be writing this series only for 3 people, including my mother. However, I do believe Alloy is an extremely valuable tool that is coming into reach for Java developers. So bear with me!
...
To keep things simple, we first focus on the `isStaticAccessible` (compile time). This then leaves out the concept of open modules.
...
The [model we just created][3] can now be used to automatically create some examples to do a sanity check. Just add:
...
* The `java.base` module is in the dependences of `Module$0` but via a ` requires static java.base;`. This was unexpected for me. It does not make sense to require `java.base` either transitively nor statically (optional).
* In this example, `java.base` and a module actually have the same package as content. Though the JPMS design document clearly makes this illegal, there is no rule in the JLS about this overlap. This will be handled in later versions when things get hairy.
...
[3]: https://raw.githubusercontent.com/pkriens/pkriens.github.io/gh-pages/jpms/compile-only.als
